...
def _find_best_arb_for_combination(
    ...
    for name, keys in MARKET_SETS.items():
        ...
        if arb is not None and arb < best_arb_percentage:
            ...
            # 1. Always check the activity tracker first. This is the source of truth.
            ...
            # 2. If we still don't have a source, run the chosen detection method.
            if not misvalue_source:
                if MISVALUE_DETECTION_METHOD == "appearance":
                    # This method runs once to determine the source and saves it to activity_data.
                    # It will only return a value on the first-ever appearance.
                    ...
                    # This method runs every time, as it's stateless.
                    misvalue_source = _identify_misvalue_source(opportunity, all_matches_in_group)

            # 3. If a source was found, attach it to both the opportunity AND activity_data immediately
            if misvalue_source:
                opportunity['misvalue_source'] = misvalue_source
                # CRITICAL: Save to activity_data immediately, regardless of confirmation status
                if unique_id:
                    activity_data.setdefault(unique_id, {})['misvalue_source'] = misvalue_source
            # ---------------------------------
